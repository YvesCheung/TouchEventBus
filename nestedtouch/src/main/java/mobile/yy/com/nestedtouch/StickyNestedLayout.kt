...
class StickyNestedLayout : LinearLayout,
    ...
    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        ...
        setMeasuredDimension(measuredWidth,
                headView.measuredHeight + navView.measuredHeight + contentView.measuredHeight)
    }
    ...
    private fun scrollToWithUnConsumed(dx: Int, dy: Int, unconsumed: IntArray? = null) {
        when {
            ...
            dy > headView.measuredHeight -> {
                scrollTo(dx, headView.measuredHeight)
                unconsumed?.set(1, dy - headView.measuredHeight)
            }
            else -> scrollTo(dx, dy)
        }
    ...
    override fun onStartNestedScroll(child: View, target: View, nestedScrollAxes: Int): Boolean {
        ...
        startNestedScroll(nestedScrollAxes) //开始通知parent的嵌套滑动
        return true
    }
    ...
    override fun onNestedPreScroll(target: View, dx: Int, dy: Int, consumed: IntArray) {
        ...
        val headViewScrollDis = headView.measuredHeight - scrollY
        ...
        if (headViewCanBeExpand) {
            log {
                "dx = $dx dy = $dy parentConsumed = ${parentConsumed[0]},${parentConsumed[1]}" +
                        ...
                        " headViewHeight = ${headView.measuredHeight}"
            }

            if (leftY > headViewScrollDis) { //滑的距离超过了能滚的距离
                scrollBy(0, headViewScrollDis)
                consumed[1] = headViewScrollDis + parentConsumed[1]  //只消费能滚的最大距离
            } else {
                scrollBy(0, leftY) //没超过滚的极限距离，那就滑多少滚多少
    ...
    override fun onNestedScroll(target: View, dxConsumed: Int, dyConsumed: Int,
        ...
        if (dyUnconsumed < 0 && scrollY >= 0) { //下滑而且headView能向下滚
            log {
                "dxC = $dxConsumed dyC = $dyConsumed dxU = $dxUnconsumed dyU = $dyUnconsumed"
            }

            val headViewCanScroll = scrollY
            if (headViewCanScroll < Math.abs(dyUnconsumed)) { //滑动距离超过了可以滚的范围
                scrollBy(0, -headViewCanScroll) //只滚我能滚的
                dyUnconsumedAfterMe = dyUnconsumed + headViewCanScroll //只消费我能滑的
                dyConsumedAfterMe = dyConsumed - headViewCanScroll
            } else { //全部都能消费掉
