 ...
 * 个Handler等等。</p>
 * <p>每个Handler自己定义需要拦截哪些Handler。当触摸事件发生时，TouchEventBus并不是根据View树的顺序来分发事件，
 * 而是根据业务的需要，逐个Handler分发。通过{@link #nextHandler()}方法给出在此业务下方的Handler列表。那么
 * 当前Handler就会先于它们接收到触摸事件，从而判断是否需要拦截或者放开。</p>
 * <p>在{@link #onTouch(Object, MotionEvent, boolean)}方法写对触摸事件处理的业务。</p>
 * <p>如果当前Handler非常的霸道，无论前面的Handler是否已经拦截处理了触摸事件，自己都还是要收到触摸事件，那么就让
 * {@link #forceMonitor()}返回true。</p>
 */
public interface TouchEventHandler<VIEW, HOLDER extends TouchViewHolder<VIEW>> {
     ...
     *
     * @return 获取当前Handler的 {@link TouchViewHolder}
     */
    @NonNull
